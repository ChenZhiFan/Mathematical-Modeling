...
# 一般的非线性规划问题
> 对于一般的非线性规划问题，由于不是凸优化，就不能使用[6. Cvxpy](6.%20Cvxpy.md)求解，求解一般使用[3. Sympy](3.%20Sympy.md)中的`optimize.minimize`函数求解
...
1. 建筑工地的位置（单位$\mbox{km}$）及其水泥日用量$c$（单位$t$）。拟建造两个料场向6个工地运送水泥，两个料场日储存量各为$20t$，问料场建在何处，使总的吨公里数最小
   ...
   建立模型如下：
   $$
   \min \sum_{i=1}^6\sum_{j=1}^2\sqrt{(x_j-a_i)^2+(y_j-b_i)^2},
   $$
   $$
   s.t.
   \begin{cases}
   ...
   z_{ij}\le0,\quad i=1,2,\cdots,6;j=1,2.
   \end{cases}
   $$
   ```python
